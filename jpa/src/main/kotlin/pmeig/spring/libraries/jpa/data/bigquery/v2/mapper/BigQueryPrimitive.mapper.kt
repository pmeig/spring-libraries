...
internal enum class BigQueryPrimitive(private val type: StandardSQLTypeName,
  ...
  companion object {
    fun from(type: StandardSQLTypeName, target: KClass<*>? = null): BigQueryPrimitive? =
      target?.let { clazz -> entries.find { it.type == type && it.target == clazz } } ?: entries.find { it.type == type }
  }
}
